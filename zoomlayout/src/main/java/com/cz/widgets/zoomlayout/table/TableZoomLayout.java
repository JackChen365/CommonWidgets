package com.cz.widgets.zoomlayout.table;
...
public class TableZoomLayout extends RecyclerZoomLayout {
    ...
    protected int scrollHorizontallyBy(int dx,boolean isScaleDragged) {
        ...
        if(0 < childCount){
            int width = orientationHelper.getTotalSpaceInOther();
            View startChildView = getChildAt(0);
            View lastChildView = getChildAt(childCount - 1);
            int layoutDirection = 0 > dx ? DIRECTION_START : DIRECTION_END;

            int paddingLeft = getPaddingLeft();
            int left = getDecoratedLeft(startChildView);
            int decoratedRight = getDecoratedRight(lastChildView);
            int right = decoratedRight - width;
            if (DIRECTION_START == layoutDirection) {
                //to left
                if(left>paddingLeft){
                    scrolled=0;
                } else if (left-dx > paddingLeft) {
                    scrolled = left-paddingLeft;
                } else if (right - dx < 0) {
                    scrolled = right;
                }
            ...
            if(0 != scrolled){
                scrolled=offsetChildrenHorizontal(-scrolled);
            }
        }
        return scrolled;
    }
    ...
    protected int scrollVerticallyBy(int dy,boolean isScaleDragged) {
        ...
        int childCount = getChildCount();
        if(0 < childCount && 0 < consumed){
            View lastChildView = getChildAt(childCount - 1);
            int totalSpace = orientationHelper.getTotalSpace();
            int decoratedBottom = getDecoratedBottom(lastChildView);
            if(decoratedBottom<totalSpace){
                consumed = decoratedBottom-totalSpace;
            } else {
