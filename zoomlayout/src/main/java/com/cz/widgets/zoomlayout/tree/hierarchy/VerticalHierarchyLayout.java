package com.cz.widgets.zoomlayout.tree.hierarchy;
 ...
 */
public class VerticalHierarchyLayout extends RecyclerZoomLayout implements Previewable {
    ...
    public VerticalHierarchyLayout(Context context) {
        this(context,null, R.attr.hierarchyLayout);
    }

    public VerticalHierarchyLayout(Context context, AttributeSet attrs) {
        this(context, attrs, R.attr.hierarchyLayout);
    }

    public VerticalHierarchyLayout(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        if(null!=adapter&&layoutState.structureChanged){
            ...
            int left=paddingTop+layoutState.scrollX+hierarchyNode.centerBreadth * decoratedMeasuredWidth;
            int top=paddingLeft+hierarchyNode.depth*decoratedMeasuredHeight;
            ...
            int totalSpace = orientationHelper.getTotalSpaceInOther();
            //Update the scroll location. We move the node to the vertical center.
            dispatchScrollChange(left-(totalSpace-decoratedMeasuredWidth)/2,0);
            layoutState.scrollTo(left-(totalSpace-decoratedMeasuredWidth)/2,0);
            //Fill the window
            fillHierarchyLayout();
        }
    }
    ...
    protected int scrollHorizontallyBy(int dx, boolean isScaleDragged) {
        ...
        int measuredWidth = treeNodeIndexer.getTreeMeasuredWidth();
    ...
    protected int scrollVerticallyBy(int dy, boolean isScaleDragged) {
        ...
        int measuredHeight = treeNodeIndexer.getTreeMeasuredHeight();
        int scrolled = dy;
        if(top < paddingTop){
            //to top
    ...
    public View newPreview() {
        ...
        VerticalHierarchyLayout layout=new VerticalHierarchyLayout(context){
            ...
            protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
                ...
                if(null!=adapter&&null!=adapter.root){
                    TreeNode hierarchyNode = adapter.root;
                    childView = getViewAndMeasured(hierarchyNode);
...
//                adapter.onBindView(context,childView,hierarchyNode,hierarchyNode.value);
                ...
                int decoratedMeasuredWidth = getDecoratedMeasuredWidth(child);
                int decoratedMeasuredHeight = getDecoratedMeasuredHeight(child);
                int left=paddingTop+hierarchyNode.centerBreadth * decoratedMeasuredWidth;
                int top=paddingLeft+hierarchyNode.depth * decoratedMeasuredHeight;
                layoutDecorated(child,left,top,left+decoratedMeasuredWidth,top+decoratedMeasuredHeight);
                canvas.save();
