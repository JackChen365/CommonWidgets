 ...
 */
public class HierarchyTestLayout extends RecyclerZoomLayout implements Previewable {
    ...
    public HierarchyTestLayout(Context context) {
        this(context,null, R.attr.hierarchyLayout);
    }

    public HierarchyTestLayout(Context context, AttributeSet attrs) {
        this(context, attrs, R.attr.hierarchyLayout);
    }

    public HierarchyTestLayout(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        if(null!=adapter&&layoutState.structureChanged){
            ...
            treeNodeIndexer=new TreeNodeIndexer(hierarchyNode,TreeNodeIndexer.HORIZONTAL,decoratedMeasuredWidth,decoratedMeasuredHeight);

            int left=paddingLeft+hierarchyNode.depth*decoratedMeasuredWidth;
            int top=paddingTop+layoutState.scrollX+hierarchyNode.centerBreadth * decoratedMeasuredHeight;
            ...
            dispatchScrollChange(0,top-(totalSpace-decoratedMeasuredHeight)/2);
            layoutState.scrollTo(0, top-(totalSpace-decoratedMeasuredHeight)/2);
            //Fill the window
            fillHierarchyLayout();
        }
    }
    ...
    private<E> View addHierarchyViewInternal(TreeNode<E> hierarchyNode){
        ...
        int left=paddingLeft+hierarchyNode.depth * decoratedMeasuredWidth;
        int top=paddingTop+hierarchyNode.centerBreadth * decoratedMeasuredHeight;
    ...
    private void fillHierarchyLayout() {
        ...
        int left = treeNodeIndexer.findHierarchyDepthIndex(hintRect.left);
        int right = treeNodeIndexer.findHierarchyDepthIndex(hintRect.right);

        int top = treeNodeIndexer.findHierarchyBreadthIndex(hintRect.top);
        int bottom = treeNodeIndexer.findHierarchyBreadthIndex(hintRect.bottom);
    ...
    protected int scrollHorizontallyBy(int dx, boolean isScaleDragged) {
        ...
        int leftHierarchyDepthIndex =treeNodeIndexer.findHierarchyDepthIndex(left);
        int rightHierarchyDepthIndex = treeNodeIndexer.findHierarchyDepthIndex(right);
        ...
        Log.i(TAG,"scrollHorizontallyBy:"+dx+" scrollX:"+layoutState.scrollX+" scrollY:"+layoutState.scrollY+" left:"+leftHierarchyDepthIndex+" right:"+rightHierarchyDepthIndex+" rect:"+layoutState.rect+" total:"+totalSpaceInOther);
    ...
    protected int scrollVerticallyBy(int dy, boolean isScaleDragged) {
        ...
        int topHierarchyBreadthIndex = treeNodeIndexer.findHierarchyBreadthIndex(top);
        int bottomHierarchyBreadthIndex = treeNodeIndexer.findHierarchyBreadthIndex(bottom);
        if(isScaleDragged||DIRECTION_START==layoutDirection){
            //Move backward.
        ...
        Log.i(TAG,"scrollVerticallyBy:"+dy+" scrollX:"+layoutState.scrollX+" scrollY:"+layoutState.scrollY+" top:"+topHierarchyBreadthIndex+" bottom:"+bottomHierarchyBreadthIndex+" rect:"+layoutState.rect+" isScaleDragged:"+isScaleDragged);
        //Recycler view by layout state.
        recycleByLayoutState(layoutState);
        //Offset all the child views.
    ...
    private void recycleByLayoutState(LayoutState layoutState) {
        ...
        for(int i=0;i<getChildCount();){
            ...
            if(null!=treeNode){
                int x = treeNode.depth;
                //We use center breadth to layout.
                int y = treeNode.centerBreadth;
                if(rect.left <= rect.right && rect.top <= rect.bottom  // check for empty first
                    ...
                    Log.i(TAG,"recycleByLayoutState:"+treeNode.depth+" breadth:"+treeNode.centerBreadth);
    ...
    private void fillHierarchyLayout(int left,int top,int right,int bottom){
        for(int row=top;row<=bottom;row++){
            for(int column= left;column<=right;column++){
                TreeNode treeNode = treeNodeIndexer.findTreeNode(row, column);
                if(null!=treeNode){
    ...
    public View newPreview() {
        ...
        HierarchyTestLayout layout=new HierarchyTestLayout(context){
            ...
            protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
                ...
                if(null!=adapter&&null!=adapter.root){
                    ...
                    //Initialize the indexer
                    TreeNodeIndexer treeNodeIndexer=new TreeNodeIndexer(hierarchyNode,TreeNodeIndexer.HORIZONTAL,decoratedMeasuredWidth,decoratedMeasuredHeight);
